...
$(function () {
	...
	var musixMatchAPIKey = "157850154be4d64a8aa51557d0b980b6";
	var song = "";
	...
	function displayMusicMatch(response) {
		...
		for (countMM = 0; countMM < resultsNUM; countMM++) {
			...
			var songFromMM = JSON.parse(response).message.body.track_list[countMM].track.track_name;

			//Assigns artist from MusicMatch
			var artistFromMM = JSON.parse(response).message.body.track_list[countMM].track.artist_name;

			// Log the ID for first result
			var idFromMM = JSON.parse(response).message.body.track_list[countMM].track.track_id;
		...
		$(".songBtn").click(function (event) {
			...
			var selectedSong = $(this).children("#songName").text();
			// ArtistName on selected button
			var selectedArtist = $(this).children("#artistName").text();
	...
	function displayDeezer(result) {
		...
		$("#artistBox").parent().show();
		// display audio control to play sample of selected song
		var playControl = '<audio controls><source src="' + playSample + '" type="audio/mpeg"></audio>';
